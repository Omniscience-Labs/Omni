...
interface AgentSelectionState {
  ...
  isOmniAgent: (agents: Agent[]) => boolean;
}
  ...
  persist(
    (set, get) => ({
      ...
      initializeFromAgents: (agents: Agent[], threadAgentId?: string, onAgentSelect?: (agentId: string | undefined) => void) => {
        ...
        if (!selectedId) {
          ...
          if (current && agents.some(a => a.agent_id === current)) {
            ...
            const defaultOmniAgent = agents.find(agent => agent.metadata?.is_suna_default);
            selectedId = defaultOmniAgent ? defaultOmniAgent.agent_id : agents[0].agent_id;
          }
        }
      ...
      autoSelectAgent: (agents: Agent[], onAgentSelect?: (agentId: string | undefined) => void, currentSelectedAgentId?: string) => {
        ...
        const defaultOmniAgent = agents.find(agent => agent.metadata?.is_suna_default);
        const agentToSelect = defaultOmniAgent || agents[0];
...
export const useIsOmniAgentFn = () => 
  useAgentSelectionStore((state) => state.isOmniAgent);

...
export const EnhancedPublishDialog = ({
  ...
  const handleSelectAll = () => {
    ...
    setPreferences({
      ...
      include_default_files: newValue,
    });
  };
  ...
  const componentItems = [
    ...
    {
      key: 'include_default_files' as keyof SharingPreferences,
      icon: FileText,
      title: 'Default Files',
      description: 'Files automatically loaded into agent workspace'
    }
  ];
            ...
            {!someSelected && (
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <p className="text-xs text-amber-700">
                  At least one component must be selected to publish a template
                </p>
              </div>
            )}
          </div>

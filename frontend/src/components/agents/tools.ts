export const AGENTPRESS_TOOL_DEFINITIONS: Record<string, { enabled: boolean; description: string; icon: string; color: string }> = {
    ...
    'custom_automation_tool': { enabled: false, description: 'Upload and run custom Playwright automation scripts with custom Chrome profiles', icon: '🤖', color: 'bg-violet-100 dark:bg-violet-800/50' },
...
export const getToolDisplayName = (toolName: string): string => {
    const displayNames: Record<string, string> = {
      ...
      'custom_automation_tool': 'Custom Automation',

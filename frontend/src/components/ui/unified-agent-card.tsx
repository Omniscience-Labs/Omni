...
const CardAvatar: React.FC<{
  ...
  const isOmniAgent = data.metadata?.is_suna_default === true;
...
const AgentBadges: React.FC<{ data: BaseAgentData, isOmniAgent: boolean }> = ({ data, isOmniAgent }) => (
  <div className="flex gap-1">
    {!isOmniAgent && data.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {data.current_version.version_name}
      </Badge>
    )}
    {!isOmniAgent && data.is_public && (
...
export const UnifiedAgentCard: React.FC<UnifiedAgentCardProps> = ({
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        case 'marketplace':
          return <MarketplaceBadge isOmniTeam={data.is_omni_team} isOwner={isOwner} />;
        case 'template':
          return <TemplateBadge isPublic={data.is_public} />;
        case 'agent':
          return <AgentBadges data={data} isOmniAgent={isOmniAgent} />;
        default:
          return null;
      }
    };
          ...
          <div className="flex flex-col gap-1 text-xs text-muted-foreground">
            ...
            {data.marketplace_published_at && (
              <div className="flex items-center gap-1 text-[10px] text-muted-foreground/70">
                ...
                <span>Published {formatPublishedDate(data.marketplace_published_at)}</span>
              </div>
            )}
          </div>
        );

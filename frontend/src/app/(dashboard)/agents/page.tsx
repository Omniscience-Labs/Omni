...
import { PublishDialog, PublishOptions } from '@/components/agents/custom-agents-page/publish-dialog';
...
export default function AgentsPage() {
  ...
  const marketplaceQueryParams = useMemo(() => {
    ...
    if (marketplaceFilter === 'kortix') {
      params.is_omni_team = true;
    } else if (marketplaceFilter === 'community') {
      params.is_omni_team = false;
    ...
    try {
      ...
      if (isAgent) {
        ...
        const result = await createTemplateMutation.mutateAsync({
          ...
          make_public: true,
          usage_examples: options.usageExamples,
          description: options.description,
          include_system_prompt: options.includeSystemPrompt,
          include_tools: options.includeTools,
          include_triggers: options.includeTriggers
        });

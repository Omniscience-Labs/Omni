#!/usr/bin/env python3
"""
Omni Default Agent Management Script (Simplified)

This script provides administrative functions for managing Omni default agents across all users.

Usage:
    # 🚀 MAIN COMMANDS
    python manage_suna_agents.py install-all          # Install Omni for all users who don't have it
    python manage_suna_agents.py stats                # Show Omni agent statistics
    python manage_suna_agents.py install-user <id>    # Install Omni for specific user
...
Note: Sync is no longer needed - Omni agents automatically use the current configuration from config.py
"""
...
class OmniAgentManager:
    def __init__(self):
        self.service = OmniDefaultAgentService()
    ...
    async def install_user(self, account_id):
        ...
        if agent_id:
            print(f"✅ Successfully installed Omni agent {agent_id} for user {account_id}")
        else:
            print(f"❌ Failed to install Omni agent for user {account_id}")
    ...
    async def replace_user_agent(self, account_id):
        ...
        if agent_id:
            print(f"✅ Successfully replaced Omni agent {agent_id} for user {account_id}")
        else:
            print(f"❌ Failed to replace Omni agent for user {account_id}")
    
    async def show_stats(self):
        """Show Omni agent statistics"""
        print("📊 Omni Default Agent Statistics")
        print("=" * 50)
...
async def main():
    parser = argparse.ArgumentParser(
        description="Manage Omni default agents across all users (Simplified)",
    ...
    # Main commands
    subparsers.add_parser('install-all', help='Install Omni agent for all users who don\'t have it')
    subparsers.add_parser('stats', help='Show Omni agent statistics')
    subparsers.add_parser('config-info', help='Show information about Omni configuration')
    
    # User-specific commands
    install_user_parser = subparsers.add_parser('install-user', help='Install Omni agent for specific user')
    install_user_parser.add_argument('account_id', help='Account ID to install Omni for')
    
    replace_user_parser = subparsers.add_parser('replace-user', help='Replace Omni agent for specific user (if corrupted)')
    replace_user_parser.add_argument('account_id', help='Account ID to replace Omni for')

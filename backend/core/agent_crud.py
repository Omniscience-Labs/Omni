...
async def update_agent(
    ...
    try:
        ...
        # Handle model - resolve via model manager if provided
        current_model = current_version_data.get('model')
        if agent_data.model is not None:
            from core.ai_models import model_manager
            resolved_model = model_manager.resolve_model_id(agent_data.model)
            current_model = resolved_model
            logger.debug(f"Model update: '{agent_data.model}' resolved to '{resolved_model}'")

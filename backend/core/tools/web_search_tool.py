...
class SandboxWebSearchTool(SandboxToolsBase):
    ...
    def __init__(self, project_id: str, thread_manager: ThreadManager):
        ...
        if not self.firecrawl_url:
            self.firecrawl_url = "https://api.firecrawl.dev"

        # Ensure url does not end with /
        if self.firecrawl_url.endswith("/"):
            self.firecrawl_url = self.firecrawl_url[:-1]

        # Ensure URL has protocol
        if not (
            self.firecrawl_url.startswith("http://")
            or self.firecrawl_url.startswith("https://")
        ):
            self.firecrawl_url = "https://" + self.firecrawl_url

        if not self.tavily_api_key:
            logging.warning(
                "TAVILY_API_KEY not configured - Web Search Tool will not be available"
            )
        if not self.firecrawl_api_key:
            logging.warning(
                "FIRECRAWL_API_KEY not configured - Web Scraping Tool will not be available"
            )
    ...
    async def web_search(
        self, query: str | list[str], num_results: int = 5
    ) -> ToolResult:
        """
        ...
        try:
            ...
            if is_batch:
                ...
                if not query or len(query) == 0:
                    return self.fail_response(
                        "At least one search query is required in the batch."
                    )

                # Filter out empty queries
                queries = [
                    q.strip() for q in query if q and isinstance(q, str) and q.strip()
                ]
                if not queries:
                    return self.fail_response(
                        "No valid search queries provided in the batch."
                    )

                logging.info(
                    f"Executing batch web search for {len(queries)} queries with {num_results} results each"
                )

                # Execute all searches concurrently
                start_time = time.time()
                tasks = [self._execute_single_search(q, num_results) for q in queries]
                search_results = await asyncio.gather(*tasks, return_exceptions=True)
                elapsed_time = time.time() - start_time
                logging.info(
                    f"Batch search completed in {elapsed_time:.2f}s (concurrent execution)"
                )
                ...
                for i, result in enumerate(search_results):
                    if isinstance(result, Exception):
                        logging.error(
                            f"Error processing query '{queries[i]}': {str(result)}"
                        )
                ...
                logging.info(
                    f"Batch search completed: {len([r for r in batch_response['results'] if r.get('success')])}/{len(queries)} queries successful"
                )
            ...
            logging.error(
                f"Error performing web search for '{query_str}': {error_message}"
            )
    ...
    async def _execute_single_search(self, query: str, num_results: int) -> dict:
        ...
        try:
            ...
            logging.info(
                f"Retrieved search results for query: '{query}' - {len(results)} results, answer: {'yes' if answer else 'no'}"
            )
    ...
    async def scrape_webpage(self, urls: str, include_html: bool = False) -> ToolResult:
        """
        ...
        try:
            ...
            if not self.firecrawl_api_key:
                return self.fail_response(
                    "Web Scraping is not available. FIRECRAWL_API_KEY is not configured."
                )
            ...
            elapsed_time = time.time() - start_time
            logging.info(
                f"Scraped {len(url_list)} URLs in {elapsed_time:.2f}s (concurrent execution)"
            )
    ...
    async def _scrape_single_url(self, url: str, include_html: bool = False) -> dict:
        ...
        try:
            ...
            # Ensure we can extract a domain even if protocol is missing
            parse_url = url
            if not (
                parse_url.startswith("http://") or parse_url.startswith("https://")
            ):

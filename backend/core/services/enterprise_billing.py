...
class SimplifiedEnterpriseBillingService:
    ...
    async def negate_credits(
        ...
        if not description:
            raise ValueError("Description is required for credit negation")
        ...
        try:
            ...
            # Call the database function
            result = await client.rpc('negate_enterprise_credits', {
                'p_amount': amount,
                'p_description': description,
                'p_performed_by': performed_by
            }).execute()
            ...
            if result and hasattr(result, 'data') and result.data and len(result.data) > 0:
                response = result.data[0]
                logger.info(
                    f"Negated ${amount:.2f} enterprise credits",
                    amount=amount,
                    new_balance=response['new_balance'],
                    performed_by=performed_by
                )
                return {
                    'success': True,
                    'new_balance': response['new_balance'],
                    'amount_negated': amount
                }
            
            return {'success': False, 'error': 'Failed to negate credits'}
